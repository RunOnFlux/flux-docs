# Dashboard

The FluxCloud Dashboard provides a comprehensive overview of your decentralized cloud infrastructure, allowing you to monitor and manage your applications, resources, and network status in real-time.

## Dashboard Sections

- **[Overview](overview)** - Get a quick summary of your FluxCloud deployment status
- **[Map](map)** - View the geographic distribution of your applications across the Flux network
- **[Node List](node-list)** - Browse and manage the nodes hosting your applications
- **[Resources](resources)** - Monitor your resource usage and allocation
- **[Rewards](rewards)** - Track your earnings and rewards from the Flux network
